...
const programStart = async () => {
    ...
    while (!authenticatedCustomer) {
        ...
        if (authInfo.action === "Log In") {
            ...
            if (!exitingCustomer) {
                ...
                console.log(chalk.green(`New account created for ${signupInfo.name}`));
            }
            else {
                console.log(chalk.yellow(`\tAn account already exits for ${signupInfo.name}. Please log in.`));
            }
        }
    }
    ...
    while (continueBanking) {
        ...
        switch (actionInfo.action) {
            case "Check Balance":
                console.log(chalk.green(`\tYour current balance is: $${authenticatedCustomer.account.getBalance()}`));
                break;
            case "Deposit Money":
                ...
                console.log(chalk.green(`\t$${depositInfo.amount} has been deposited. New balance: $${authenticatedCustomer.account.getBalance()}`));
                break;
            case "Withdraw Money":
                ...
                console.log(chalk.green(`\t$${withdrawInfo.amount} has been withdraw. New balance: $${authenticatedCustomer.account.getBalance()}`));
                break;
            case "Exit":
